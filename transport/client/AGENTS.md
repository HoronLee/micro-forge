...
连接装配、endpoint 解析和通用中间件由本目录统一维护。
...
- gRPC TLS 构造通过 `security/tls` alias 复用证书加载与校验。
...
- 绕过 `endpoint.IndexByProtocol` 重复解析 `Data.Client.Services`。
- 在 HTTP/gRPC dialer 中重复实现 TLS 证书加载。
